// GERADO por tools/gerar.js a partir de src/client/comum/etiqueta.html. NÃO EDITE ESTE ARQUIVO.
/**
 * Etiqueta — interpreta o texto lido do QR da etiqueta do rolo.
 *
 * FONTE ÚNICA. Este arquivo roda no navegador (include) e é copiado para
 * src/server/dominio/Etiqueta.js por `npm run gerar` para rodar no Apps Script.
 * Não use APIs do navegador nem do Google aqui (nada de URL, window, SpreadsheetApp).
 *
 * Spec: specs/001-leitor-qr/spec.md (H4). A regra de paraFormulario é a mesma do
 * legado App.parseQrPayload (tests/fixtures/legado/parseQrPayload.js).
 */
const Etiqueta = (function () {
  'use strict';

  /** Equivalente a Object.fromEntries(new URL(texto).searchParams) sem depender de URL. */
  function parametrosDaUrl(texto) {
    const semFragmento = String(texto).split('#')[0];
    const inicio = semFragmento.indexOf('?');
    const params = {};
    if (inicio < 0) return params;
    semFragmento.slice(inicio + 1).split('&').forEach((par) => {
      if (!par) return;
      const igual = par.indexOf('=');
      const chave = igual < 0 ? par : par.slice(0, igual);
      const valor = igual < 0 ? '' : par.slice(igual + 1);
      params[decodificar(chave)] = decodificar(valor);
    });
    return params;
  }

  function decodificar(s) {
    const comEspaco = s.replace(/\+/g, ' ');
    try {
      return decodeURIComponent(comEspaco);
    } catch (e) {
      return comEspaco;
    }
  }
  ...
  /**
   * Formato usado pelo formulário legado (chaves em inglês).
   * @returns {null | {supplier_id, supplier_name, nf, product_id, lot, sup_product_id,
   *   color_id, fabric_pattern, loc, meters_supplier: number, wid: number, comp}}
   */
  function paraFormulario(raw) {
    ...
    if ((text.startsWith('{') && text.endsWith('}')) || (text.startsWith('[') && text.endsWith(']'))) {
      try {
        ...
        // formato inválido: tenta o próximo
      }
    }
    ...
    if (/^https?:\/\//i.test(text)) {
      try {
        const params = parametrosDaUrl(text);
  ...
  /**
   * Formato novo (specs/000-refatoracao/contracts/api.md → EtiquetaDados).
   * @param {string} texto  texto bruto lido do QR
   * @param {'video'|'foto'|'digitado'} origem
   * @returns {null | object}  null quando o texto não tem nenhum campo reconhecível
   */
  function interpretar(texto, origem) {
    const f = paraFormulario(texto);
    if (!f) return null;
    return {
      fornecedorId: f.supplier_id,
      fornecedor: f.supplier_name,
      nf: f.nf,
      produtoId: f.product_id,
      produtoFornecedor: f.sup_product_id,
      lote: f.lot,
      cor: f.color_id,
      padronagem: f.fabric_pattern,
      localizacao: f.loc,
      composicao: f.comp,
      metrosFornecedor: f.meters_supplier || null,
      largura: f.wid || null, // unidade a confirmar: data-model.md → Unidades
      bruto: String(texto),
      origem: origem || 'video'
    };
  }

  return { paraFormulario, interpretar, parametrosDaUrl };
})();

if (typeof module !== 'undefined' && module.exports) module.exports = Etiqueta;
